...
const MEAL_TYPES = [{ key: "petit-dej", label: "Petit-déjeuner", order: 1 }, { key: "midi", label: "Midi", order: 2 }, { key: "soir", label: "Soir", order: 3 }];
const ACT_SLOTS = [{ key: "matin", label: "Matin", order: 1 }, { key: "apres-midi", label: "Après-midi", order: 2 }, { key: "soir", label: "Soir", order: 3 }];
const INFO_CATS = [
  { key: "hotel", label: "🏨 Hébergement", icon: "🏨", placeholder: "Nom hôtel, adresse, n° réservation..." },
  { key: "transport", label: "✈️ Transport", icon: "✈️", placeholder: "Vol aller, vol retour, horaires..." },
  { key: "voiture", label: "🚗 Location de voiture", icon: "🚗", placeholder: "Agence, n° réservation, dates, lieu retrait..." },
...
export default function App() {
  ...
  // Timeline unifiée : ordre chronologique repas + activités mélangés
  const TIMELINE_ORDER = { "petit-dej": 1, "matin": 2, "midi": 3, "apres-midi": 4, "soir-meal": 5, "soir-act": 6 };
  const timeline = [
    ...dayMeals.map(m => ({ ...m, _type: "meal", _order: TIMELINE_ORDER[m.meal_type === "soir" ? "soir-meal" : m.meal_type] || 99 })),
    ...dayActs.map(a => ({ ...a, _type: "act", _order: TIMELINE_ORDER[a.slot === "soir" ? "soir-act" : a.slot] || 99 })),
  ...
  const updMeal = async (id, f, v) => { isEditing.current = true; setMeals(p => p.map(m => m.id === id ? { ...m, [f === "type" ? "meal_type" : f]: v } : m)); await api.patch("meals", id, { [f === "type" ? "meal_type" : f]: v }); isEditing.current = false; };
  ...
  const updAct = async (id, f, v) => { isEditing.current = true; setActivities(p => p.map(a => a.id === id ? { ...a, [f]: v } : a)); await api.patch("activities", id, { [f]: v }); isEditing.current = false; };
  ...
  // Notes
  const updNote = (v) => { isEditing.current = true; setNotes(p => ({ ...p, [dn]: v })); clearTimeout(noteTimer.current); noteTimer.current = setTimeout(() => { api.upsertNote(dn, v); isEditing.current = false; }, 800); };
  ...
  const updCaption = async (id, v) => { isEditing.current = true; setPhotos(p => p.map(x => x.id === id ? { ...x, caption: v } : x)); await api.patch("photos", id, { caption: v }); isEditing.current = false; };
  ...
  const updInfo = (id, f, v) => {
    ...
    infoTimer.current[id] = setTimeout(() => { api.patch("infos", id, { [f]: v }); isEditing.current = false; }, 800);
  };
    ...
    return (
      <div style={{ background: "#f0ece6", minHeight: "100dvh" }}>
        ...
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", borderTop: "1px solid #ddd8d0", padding: "10px 12px", zIndex: 80 }}>
          <button onClick={() => { isEditing.current = false; loadAll(); showToast("✅ Sauvegardé !"); }} style={{ width: "100%", padding: "14px", borderRadius: 12, border: "none", background: "#1a7a6d", color: "#fff", fontSize: "0.92rem", fontWeight: 700, cursor: "pointer", letterSpacing: 0.5 }}>💾 Sauvegarder</button>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: "Georgia, serif", fontSize: "1.08rem", fontWeight: 700 }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.95rem", background: "rgba(26,122,109,0.1)" }}>📅</div>Programme
            </div>
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={addMeal} style={{ padding: "8px 12px", border: "1.5px dashed #ddd8d0", borderRadius: 9, background: "none", fontSize: "0.72rem", fontWeight: 600, color: "#d47a2e", cursor: "pointer" }}>+ 🍽️ Repas</button>
              <button onClick={addAct} style={{ padding: "8px 12px", border: "1.5px dashed #ddd8d0", borderRadius: 9, background: "none", fontSize: "0.72rem", fontWeight: 600, color: "#2e78b5", cursor: "pointer" }}>+ 📋 Activité</button>
            </div>
          </div>
          {timeline.length === 0 && <div style={{ padding: 16, background: "#fff", borderRadius: 11, boxShadow: "0 1px 4px rgba(0,0,0,0.04)", color: "#bbb", fontSize: "0.84rem", fontStyle: "italic", textAlign: "center" }}>Aucun programme pour ce jour</div>}
          {timeline.map(item => item._type === "meal" ? (
            <Card key={"m-" + item.id}>
              <div style={{ width: 6, height: "100%", minHeight: 28, borderRadius: 3, background: "#d47a2e", flexShrink: 0 }} />
              <Sel value={item.meal_type} onChange={v => updMeal(item.id, "type", v)} options={MEAL_TYPES} color="#d47a2e" />
              <Inp placeholder="Qu'est-ce qu'on mange ?" value={item.text || ""} onChange={v => updMeal(item.id, "text", v)} />
              <Del onClick={() => delMeal(item.id)} />
            </Card>
          ) : (
            <Card key={"a-" + item.id}>
              <div style={{ width: 6, height: "100%", minHeight: 28, borderRadius: 3, background: "#2e78b5", flexShrink: 0 }} />
              <Sel value={item.slot} onChange={v => updAct(item.id, "slot", v)} options={ACT_SLOTS} color="#2e78b5" />
              <Inp placeholder="Activité, visite, RDV..." value={item.text || ""} onChange={v => updAct(item.id, "text", v)} />
              <Del onClick={() => delAct(item.id)} />
            </Card>
          ))}
        </div>
